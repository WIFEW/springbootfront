    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.peisongxingming != '') searchWhere.peisongxingming = '%' + this.formSearch.peisongxingming + '%';
	      params['sfsh'] = '是';
        this.$http.get('jiankangshangchuan/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/jiankangshangchuanDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
