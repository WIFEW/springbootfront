...
<div class="lists" :style='{"width":"100%","padding":"30px 7%","margin":"0 auto","position":"relative","background":"url(http://codegen.caihongy.cn/20230127/3dc7a96395244ef894fde9c1deda72e9.jpg) no-repeat center bottom,linear-gradient(180deg, rgba(209,238,254,1) 0%, rgba(252,253,254,1) 40%, rgba(252,253,254,1) 100%),#fcfdfe","display":"block"}'>
	...
	<div class="title" :style='{"margin":"0 auto 20px","alignItems":"center","background":"url(http://codegen.caihongy.cn/20230127/9b960911d44148759e8047661d17347b.png) no-repeat center bottom","display":"flex","width":"100%","lineHeight":"50px","justifyContent":"center"}'>
	  <span :style='{"padding":"0 0 30px","fontSize":"32px","lineHeight":"50px","color":"#111"}'>物资信息展示</span>
	</div>
	...
	<div class="list list5 index-pv1">
		<div id="lists-five-swiperwuzixinxi" class="swiper-container lists-five-swiper" :style='{"width":"100%","padding":"50px 10px","margin":"40px 0 0","background":"none"}'>
			<div class="swiper-wrapper">
				<div :style='{"border":"0","padding":"20px","fontSize":"0","position":"relative","borderRadius":"4px","background":"none"}' class="swiper-slide" v-for="(item,index) in wuzixinxiList" :key="index" @click="toDetail('wuzixinxiDetail', item)">
					<img :style='{"border":"0","width":"200px","padding":"40px 20px","background":"url(http://codegen.caihongy.cn/20230128/d927f93371f746899f81fd3ae78502d1.png) no-repeat center top / 100% 100%","height":"220px"}' v-if="item.wuzitupian.substr(0,4)=='http'" :src="item.wuzitupian.split(',')[0]" alt="" />
					<img :style='{"border":"0","width":"200px","padding":"40px 20px","background":"url(http://codegen.caihongy.cn/20230128/d927f93371f746899f81fd3ae78502d1.png) no-repeat center top / 100% 100%","height":"220px"}' v-else :src="baseUrl + (item.wuzitupian?item.wuzitupian.split(',')[0]:'')" alt="" />
				</div>
			</div>
			<!-- Add Arrows -->
	...
	<div @click="moreBtn('wuzixinxi')" :style='{"border":"0","padding":"0 20px","margin":"40px auto 50px","textAlign":"center","background":"none","display":"inline-block","width":"100%","lineHeight":"40px"}'>
		<span :style='{"color":"#3e97c6","fontSize":"18px"}'>查看更多</span>
		<i v-if="true" :style='{"color":"#3e97c6","fontSize":"18px"}' class="el-icon-d-arrow-right"></i>
	</div>
    ...
    methods: {
		...
		getList() {
          let autoSortUrl = "";
			...
			this.$http.get('wuzixinxi/list', {params: {
				sfsh: '是',
				page: 1,
				limit: 8,
			}}).then(res => {
				if (res.data.code == 0) {
					this.wuzixinxiList = res.data.data.list;
					
					// 商品列表样式五
					this.$nextTick(() => {
						new Swiper('#lists-five-swiperwuzixinxi', {
							loop: true,
							speed: 500,
